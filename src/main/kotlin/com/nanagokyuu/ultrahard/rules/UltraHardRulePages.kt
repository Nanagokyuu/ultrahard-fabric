...
internal object UltraHardRulePages {
	fun create(config: UltraHardConfig): List<Filterable<Component>> = listOf(
		...
		page("当前生命连击",
			...
			"达到 ${config.killDamageExperienceCap} 点后，敌人伤害最低为 ${number(config.killDamageMinimumMultiplier)} 倍；玩家本人或玩家投射物击杀才计入。"),
		page("连击提示",
			"每 ${config.killDamageExperienceMilestone} 点战斗经验播放粒子，并在物品栏上方显示经验和修正，不刷聊天框。",
			"超过减伤上限后不再增加经验收益，但达到节点的提示仍按实际累计经验显示。提示中的修正仅指战斗经验倍率，不包含护甲倍率。"),
		page("连击清空",
		...
		page("远程与环境",
			"骷髅保持射距；苦力怕面对举盾玩家最多尝试绕后两次，均失败后直接爆炸；引爆时其他敌人疏散。末影人尝试绕到视线后方，女巫保持后排支援。",
