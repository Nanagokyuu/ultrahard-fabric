...
internal object UltraHardRulePages {
	fun create(config: UltraHardConfig): List<Filterable<Component>> = listOf(
		...
		page("战斗经验预览",
			"实时经验预览默认隐藏。按键盘＝键显示左上角的经验数值、上限、百分比和进度条，再按一次隐藏；可在控制设置的杂项中改键。",
			"仅超困难生存或冒险模式下显示。数据以服务端为准；开关在本次游戏进程内保留，重启客户端后恢复隐藏。"),
		page("吸血条件",
		...
		page("近战战术",
			"僵尸前后包抄，未持武器者给持武器者让位；蠹虫和末影螨侧翼骚扰。",
			"绕盾失败、3秒无进展或累计6秒超时后，正面施压5秒；盾牌不是绝对安全。"),
		page("远程与环境",
			"持弓骷髅保持射距并主动包围玩家；末影人尝试绕到视线后方，女巫保持后排支援。",
		...
		page("附近玩家与射击",
			...
			"持弓骷髅不要求玩家举盾，会主动选择左侧、右侧或后方射击；多只会分散到玩家四周。"),
		page("蜘蛛扑击",
			"蜘蛛会在中距离主动蓄力扑向玩家，利用水平冲量快速缩短距离；进入近战范围后恢复原版攻击。",
			"扑击有独立冷却，并检查地面、目标距离和碰撞空间；空间不足时不会强行起跳。"),
		page("骷髅包围战术",
			"同一玩家周围24格内的持弓骷髅分配环形站位：两只夹击，四只覆盖四周。编队不随玩家转头旋转，玩家移动或成员变化后会调整。",
			"先确认落点可达且有射界，再移动并恢复射击。无路、3秒无进展或6秒未到位时，恢复进攻5秒后重试；狭窄地形可能无法包围。"),
		page("僵尸与骷髅组合",
			"同一玩家24格内同时有僵尸和持弓骷髅时，僵尸负责前后，骷髅负责左右；以编队建立时的玩家朝向为准，多只会分散站位。",
			"先检查空间和路径，狭窄走廊、低顶或墙角不强制组合。僵尸到位后恢复近战，换位受阻或超时也会继续攻击。持弓凋灵骷髅同样参与左右翼分工。"),
		page("亡灵后跳与游泳",
			"僵尸和骷髅后退或近距离接敌时，身后一格高台阶若有安全落点和足够净空，可直接向后跳上去；不会穿墙、破坏方块或跳过两格高墙。",
			"骷髅及其变种，包括凋灵骷髅，可在水中浮游并寻路。后跳与浮游仅在超困难生效；水中或骑乘时不强行后跳。"),
		page("凋灵骷髅战术",
			"未持弓凋灵骷髅使用专属近战AI。单只每次出手后尝试换侧再攻击，盾牌格挡也会触发；多只则分配近身包围站位。",
			"空间不足、不可达或换位超时就恢复普通近战，保留原版攻击冷却与凋灵效果。持弓者不占用近战包围位置。"),
		page("凋灵骷髅弓手",
			"超困难下，凋灵骷髅生成装备时有10%概率持弓，其余保留原版石剑。持弓者与普通骷髅共用射距、退避、侧翼包围和僵尸混编AI。",
			"装备只在生成时抽签，读档或切换难度不重抽。战术按当前武器切换：任一手持弓使用弓手AI，移除弓后恢复专属近战。"),
		page("怪物生成",
