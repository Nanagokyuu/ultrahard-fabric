...
internal object UltraHardRulePages {
	fun create(config: UltraHardConfig): List<Filterable<Component>> = listOf(
		page("超困难规则",
			"基于困难模式，取消食物和饱和度自然回血。药水、金苹果、睡眠和吸血仍可治疗。",
		...
		page("伤害如何计算",
			"敌人输入伤害 × 护甲倍率 × 击杀倍率，再进入原版护甲等减伤流程。",
			"默认满钻石甲为1.8倍，50击杀后为1.35倍。龙、凋灵、监守者不受这两项倍率修正；PvP不受影响。"),
		page("输出软上限",
		...
		page("当前生命连击",
			"每次有效击杀使击杀倍率降低 ${percent(config.killDamageReductionPerKill)} 个百分点，最多计 ${config.killDamageReductionMaximumKills} 次，倍率最低 ${number(config.killDamageMinimumMultiplier)}。",
			"玩家本人或玩家投射物击杀敌对生物计数；宠物击杀、纯环境死亡和PvP不计。"),
		page("连击提示",
			"每 ${config.killDamageMilestoneInterval} 次击杀播放粒子，并在物品栏上方显示击杀数和修正，不刷聊天框。",
			"超过减伤上限后，计数和周期提示继续。提示中的修正仅指击杀倍率，不包含护甲倍率。"),
		page("连击清空",
			"死亡时显示击杀总结并清零；进入创造、旁观或离开超困难时也清零。",
			"重登和正常换维度保留。不死图腾救回不算死亡。清零后击杀倍率回到1.0，护甲倍率仍单独计算。"),
		page("吸血条件",
		...
		page("远程与环境",
			"骷髅保持射距；苦力怕引爆时其他敌人疏散。末影人尝试绕到视线后方，女巫保持后排支援。",
			"亡灵白天寻找水或阴凉处；敌人不因互伤转移仇恨。护甲生成概率为原版 ${number(config.mobArmorSpawnMultiplier)} 倍。"),
	)
